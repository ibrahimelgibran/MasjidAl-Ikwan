...
class C_pemasukan extends CI_Controller
{
	...
	public function getdata()
	{
		...
		$bulanini = date('m');
		$tahunini = date('Y');
		$pemasukanbulanini = $this->db->query("SELECT  MONTH(pemasukan_tanggal) AS Month,YEAR(pemasukan_tanggal) AS tahun, SUM(pemasukan_jumlah) AS TotalBulan
        FROM tbl_pemasukan
        WHERE MONTH(pemasukan_tanggal) = '$bulanini' AND YEAR(pemasukan_tanggal) = '$tahunini'
		...
		foreach ($query->result() as $key => $lists) {
			...
			$data[$key][]  = $lists->donatur;
			$data[$key][]  = 'Rp.' . rupiah($lists->pemasukan_jumlah) . ' ';
