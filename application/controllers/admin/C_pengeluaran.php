...
class C_pengeluaran extends CI_Controller
{
	...
	public function getdata()
	{
		...
		$query = $this->db->query(" SELECT tbl_pengeluaran.*,tbl_user.`user_nama` AS namauser,tbl_headaccount.account_id as acid , tbl_headaccount.head_account as ha FROM
        tbl_pengeluaran, tbl_user, tbl_headaccount WHERE
        tbl_pengeluaran.`pengeluaran_user`= tbl_user.`user_id`  AND  tbl_pengeluaran.`head_account`= tbl_headaccount.`id`
		 Order by pengeluaran_tanggal DESC");
		...
		foreach ($query->result() as $key => $lists) {
			...
			$data[$key][]  = $lists->acid.' - '.$lists->ha;
			$data[$key][]  = 'Rp.' . rupiah($lists->pengeluaran_jumlah);
